...
export default function GameScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ========== 極簡圓形圖標組（頂部） ========== */}
      {isReady && (
        <SafeAreaView style={styles.topIconBarContainer} pointerEvents="box-none">
          <View style={styles.topIconBar}>
            {/* 戶外模式圖標 */}
            <TouchableOpacity
              style={[styles.iconButton, !isSimulatorMode && styles.iconButtonActive]}
            ...
            {/* 開發者工具圖標 */}
            <TouchableOpacity
              style={[styles.iconButton, showDevDashboard && styles.iconButtonActive]}
              onPress={() => setShowDevDashboard(!showDevDashboard)}
              activeOpacity={0.7}
            >
              <Text style={styles.iconEmoji}>{showDevDashboard ? '🔧' : '⚙️'}</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      )}
