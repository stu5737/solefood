...
open class ExpoAutolinkingSettingsExtension(
  ...
  val reactNativeGradlePlugin: File by lazy {
    File(
      settings.providers.exec { env ->
        ...
        env.commandLine("/Users/yumingliao/.nvm/versions/node/v20.19.6/bin/node", "--print", "require.resolve('@react-native/gradle-plugin/package.json', { paths: [require.resolve('react-native/package.json')] })")
  ...
  val reactNative: File by lazy {
    File(
      settings.providers.exec { env ->
        env.workingDir(projectRoot)
        env.commandLine("/Users/yumingliao/.nvm/versions/node/v20.19.6/bin/node", "--print", "require.resolve('react-native/package.json')")
      }.standardOutput.asText.get().trim(),
    ).parentFile
  }
